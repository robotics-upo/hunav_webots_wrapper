...
public:
  ...
  std::string generateAgentWBT(const hunav_msgs::msg::Agent &agent);
  bool appendAgentsToWorldFile();
  bool generateAgentURDFFiles();
  std::string populateAgentURDF(const hunav_msgs::msg::Agent &agent, const std::string &overseer_name);
...
private:
  ...
  std::string base_world_path_;
  std::string urdf_folder_;
